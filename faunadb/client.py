...
class FaunaClient(object):
    ...
    def _init_session(self):
        session = Session()
        session.mount('https://', HTTPAdapter(pool_connections=self.pool_connections,
                                              pool_maxsize=self.pool_maxsize))
        session.mount('http://', HTTPAdapter(pool_connections=self.pool_connections,
                                             pool_maxsize=self.pool_maxsize))
        ...
        if self._query_timeout_ms is not None:
            session.headers["X-Query-Timeout"] = str(self._query_timeout_ms)
    ...
    def new_session_client(self, secret, observer=None):
        ...
        if self.counter.get_and_increment() > 0:
            return self.__class__(secret=secret,
                                  domain=self.domain,
                                  scheme=self.scheme,
                                  port=self.port,
    ...
    def _perform_request(self, action, path, data, query, headers):
        ...
        req = Request(action, url, params=query, data=data, auth=self.auth, headers=headers)
...
class FaunaClientAsync(FaunaClient):
    """
    Directly communicates with FaunaDB via JSON. Same API as FaunaClient, but uses aiohttp for
    asynchronous HTTP requests. Acts as a context manager to handle closing the connection, e.g.:

        async with FaunaClientAsync("secret") as client:
            resp = await client.query(query.create_collection({"name": "new_collection"}))
            ref = resp["ref"]
    ...
    def __init__(self,
        ...
        :param pool_maxsize:
          The maximum number of connections to save in the pool.
        """
        super(FaunaClientAsync, self).__init__(secret, domain, scheme, port, timeout, observer,
                                               pool_connections, pool_maxsize, **kwargs)
        self.auth = BasicAuth(secret, "")

    def _init_session(self):
        conn = TCPConnector(limit=self.pool_maxsize)
        headers = {
            "Accept-Encoding": "gzip",
            "Content-Type": "application/json;charset=utf-8",
            "X-Fauna-Driver": "python",
    ...
    async def query(self, expression, timeout_millis=None):
        """
        Use the FaunaDB query API.
        ...
        :return: Converted JSON response.
        """
        return await self._execute("POST", "", _wrap(expression), with_txn_time=True, query_timeout_ms=timeout_millis)

    async def ping(self, scope=None, timeout=None):
        """
        Ping FaunaDB.
        """
        return await self._execute("GET", "ping", query={"scope": scope, "timeout": timeout})

    async def _execute(self, action, path, data=None, query=None, with_txn_time=False, query_timeout_ms=None):
        """Performs an HTTP action, logs it, and looks for errors."""
        if query is not None:
            query = {k: v for k, v in query.items() if v is not None}
        ...
        start_time = time()
        response = await self._perform_request(action, path, data, query, headers)
        end_time = time()
    ...
    async def _perform_request(self, action, path, data, query, headers):
        ...
        url = self.base_url + "/" + path
        data = to_json(data) if data else None
        resp = await self.session.request(action, url, params=query, data=data, auth=self.auth, headers=headers)
        return resp
