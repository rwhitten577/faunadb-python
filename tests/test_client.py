...
from tests.helpers import FaunaTestCase, FaunaAsyncTestCase
...
class ClientAsyncTest(FaunaAsyncTestCase):
    async def test_ping(self):
        old_time = self.client.get_last_txn_time()
        self.assertEqual(await self.client.ping("node"), "Scope node is OK")
        new_time = self.client.get_last_txn_time()

        self.assertEqual(old_time, new_time)  # client.ping should not update last-txn-time

    def test_query_timeout(self):
        client = FaunaClientAsync(secret="secret", query_timeout_ms=5000)
        self.assertEqual(client.get_query_timeout(), 5000)

    async def test_last_txn_time(self):
        old_time = self.client.get_last_txn_time()
    ...
    async def test_error_on_closed_client(self):
        client = FaunaClientAsync(secret="secret")
